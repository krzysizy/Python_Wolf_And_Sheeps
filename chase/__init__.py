...
def json_export(dictionary_list, filename):
    f = open(filename, "w")
    f.write(json.dumps(dictionary_list, indent=4))
    logging.debug("json_export(" + str(dictionary_list) + ", " + filename
                  + "), return: ")
    f.close()
...
def simulation(rounds, sheep_num, init_pos_limit, sheep_move_dist, wolf_move_dist, wait, path):
    ...
    while round_num <= rounds and alive_sheep(sheep) != 0:
        ...
        for i in sheep:
            if i.alive_or_dead:
                dictionary["sheep_pos"].append([i.x, i.y])
            else:
                dictionary["sheep_pos"].append(None)
        json_dictionary_list.append(dictionary)
        ...
        round_num += 1
    json_export(json_dictionary_list, create_file_path(path, 'pos.json'))
...
def main():
    ...
    if args.log_lvl:
        ...
        logging.basicConfig(level=lvl, filename=create_file_path(path, 'chase.log'), filemode='w',
                            force=True)
        logging.debug("logging config")

    simulation(rounds, sheep_num, init_pos_limit, sheep_move_dist, wolf_move_dist, wait, path)